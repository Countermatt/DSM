...
//ssh -------------------------------------------------------------------------
           ...
           ssh_arg_config(exec, list_machine, argv, argc, i, char_port);
      	   /* jump to new prog : */
           // char *arvpm[]={"truc","test",NULL};
      	   //  execvp("./bin/truc",arvpm);
           ...
           for (i = 0; i < 2*nb_proc; i++) {
            ...
            case POLLNVAL:
             ...
             if (fds[i].revents==17) {
               memset(buffer,0,1024*sizeof(*buffer));
               read(fds[i].fd,buffer,1024*sizeof(*buffer));
               fprintf(stdout,"[Proc i:%d fd:%d : stdout] %s \n",i,fds[i].fd,buffer);
