...
void connexion_ssh(char *exec, char **list_machine, char *argv[], int argc, int ind, char char_port[100]){
  int k;
...
//clé ssh privé du père------------------------------------------
  char id_rsa[strlen(path)+strlen(argv[0])+strlen("/")+strlen("/../..")+strlen("/.ssh/id_rsa")];
  memset(id_rsa,0,sizeof(id_rsa));
  strcat(id_rsa, path);
  strcat(id_rsa, "/");
  strcat(id_rsa, argv[0]);
  id_rsa[strlen(id_rsa)-8] = '\0';
  strcat(id_rsa, "/../..");
  strcat(id_rsa, "/.ssh/id_rsa");

//commande à lancer----------------------------------------------
  char script[strlen(argv[0])+strlen("/")+strlen(path)+strlen("/bin/dsmwrap")];
  memset(script,0,sizeof(script));
  strcat(script, path);
  strcat(script, "/");
  strcat(script, argv[0]);
  script[strlen(script)-7] = '\0';
  strcat(script, "dsmwrap");
...
ssh_arg[10] = NULL;
fflush(stdout);
// if(execvp("ssh",ssh_arg); == -1){
//   perror(ssh_arg);
//   exit(0);
//   }
// }

if(execvp(script,ssh_arg+4) == -1){
  perror(full_path_exec);
  exit(0);
  }
}
  ...
  for(i = 0; i < nb_proc ; i++){/* recuperation des donnes*/
   ...
   for ( k = 0; k < nb_proc; k++) {
     if ( proc_array[k].pid == proc_pid) {
       ...
       write(sockFdProc[i],&(proc_array[k].connect_info.rank),sizeof(int));
       break;
     }
   }
  }
